...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run semantic segmentation on frames from a cv2 video capture source."
    )
    parser.add_argument(
        "--video-source",
        type=str,
        default=config.root_folder + "/data/videos/kitti06/video_color.mp4",
        help="OpenCV video source (camera index like '0' or video path).",
    )
