...
def extract_multiscale_features():
    ...
    parser.add_argument(
        "--num-points", type=int, default=1500, help="The number of desired features to extract."
    )
    ...
    def extract_features(image):
        ...
        pyramid = pyramid_gaussian(
            image, max_layer=args.pyramid_levels, downscale=args.scale_factor_levels
        )
    ...
    with tf.Graph().as_default():
        ...
        for idx_level in range(levels):
            ...
            point_level.append(
                args.num_points * factor_points ** (-1 * (idx_level - args.upsampled_levels))
            )
