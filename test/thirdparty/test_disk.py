...
class SceneDataset:
    def __init__(self, image_path, crop_size=(None, None)):
        ...
        self.names = [p for p in os.listdir(image_path) if p.endswith(args.image_extension)]
...
def extract(dataset, save_path):
    ...
    if args.mode == "nms":
        extract = partial(model.features, kind="nms", window_size=args.window, cutoff=0.0, n=args.n)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--f16", action="store_true", help="Store descriptors in fp16 (half precision) format"
    )
    ...
    default_model_path = (
        os.path.split(os.path.abspath(__file__))[0] + "/../../thirdparty/disk/depth-save.pth"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        "--image_path", default="../data/graf", help="Directory with images to be processed."
    )
