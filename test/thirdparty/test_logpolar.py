...
def extract_descriptors(input_filename, output_filename, use_log_polar, num_keypoints, verbose):
    # Setup
    ...
    for i, p in enumerate(pts):
        _p = (
            2 * np.array([(p[0] + padLeft) / (cfg.TEST.PAD_TO), (p[1] + padUp) / (cfg.TEST.PAD_TO)])
            - 1
        )
        kp_norm.append(_p)
    ...
    descriptors, patches = model(
        {input_filename: imgs}, img_keypoints, [input_filename] * len(img_keypoints[0])
    )
    if verbose:
        print("-- Computed {} descriptors in {:0.2f} sec.".format(descriptors.shape[0], time() - t))

    keypoints_array = np.concatenate([pts, scales[..., None], oris[..., None]], axis=1)

    t = time()
    with h5py.File(output_filename, "w") as f:
        f["keypoints"] = keypoints_array
        f["descriptors"] = descriptors.cpu().detach().numpy()
        print("-- Saved {} descriptors in {:0.2f} sec.".format(descriptors.shape[0], time() - t))


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--input", type=str, default=logpolar_base_path + "testImg.jpeg", help="Input image"
    )
    parser.add_argument(
        "--output", type=str, default=logpolar_base_path + "testImg.h5", help="Output file"
    )
    parser.add_argument(
        "--use_log_polar",
        type=bool,
        default=True,
        help="Use log-polar models. Set to False to use " "cartesian models instead.",
    )
    parser.add_argument("--num_keypoints", type=int, default=1024, help="Number of keypoints")
    parser.add_argument(
        "--verbose", type=bool, default=True, help="Set to False to suppress feedback"
    )
    ...
    if len(unparsed) > 0:
        ...
        extract_descriptors(
            config.input, config.output, config.use_log_polar, config.num_keypoints, config.verbose
        )
