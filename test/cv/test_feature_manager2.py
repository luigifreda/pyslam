...
img_kpsd = cv2.drawKeypoints(
    img, kpsd, None, color=(0, 255, 0), flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS
)

fig1 = MPlotFigure(img_kps[:, :, [2, 1, 0]], title="detected keypoints")
fig2 = MPlotFigure(
    img_kpsd[:, :, [2, 1, 0]], title="computed features (keypoints after computing descriptors)"
)
MPlotFigure.show()
