...
from pyslam.slam.pose_estimator import PoseEstimatorType, pose_estimator_factory, PoseEstimatorInput
...
def generate_random_points_2d(width, height, num_points, margin_pixels=3):
    ...
    points_2d = np.random.uniform(
        low=[margin_pixels, margin_pixels],
        high=[width - margin_pixels, height - margin_pixels],
        size=(num_points, 2),
    ).astype(np.float32)
    return points_2d
...
class TestPoseEstimators(unittest.TestCase):
    ...
    def setUp(self):
        ...
        tc1w = np.array([0.0, 0.0, 0.0], dtype=np.float32).reshape(
            (3, 1)
        ...
        tc2w = np.array([tx, ty, tz], dtype=np.float32).reshape(
            (3, 1)
        ...
        points_3d_c1, depth1 = random_backproject_points(
            self.K1, points_2d_c1, self.w1, self.h1, min_depth, max_depth
        )
        ...
        points_2d_c1_noisy = points_2d_c1 + np.random.normal(
            0, self.sigma_noise_pixel, points_2d_c1.shape
        )
        ...
        if False:
            points_2d_c1_check, mask_c1, depth1_check = project_points(
                self.K1, (self.Rc1w @ points_3d_w.T + self.tc1w).T, self.w1, self.h1
            )
            average_projection_error_c1 = np.mean(
                np.linalg.norm(
                    points_2d_c1[mask_c1 == 1, :] - points_2d_c1_check[mask_c1 == 1, :], axis=1
                )
            )
            ...
            points_2d_c2_check, mask_c2, depth2_check = project_points(
                self.K2, (self.Rc2w @ points_3d_w.T + self.tc2w).T, self.w2, self.h2
            )
            average_projection_error_c2 = np.mean(
                np.linalg.norm(
                    points_2d_c2[mask_c2 == 1, :] - points_2d_c2_check[mask_c2 == 1, :], axis=1
                )
            )
            print(
    ...
    def test_essential_matrix_2d2d(self):
        estimator = pose_estimator_factory(
            PoseEstimatorType.ESSENTIAL_MATRIX_2D_2D, self.K1, self.K2
        )
        R21, t21, inliers = estimator.estimate(
            PoseEstimatorInput(kpts1=self.kpts1, kpts2=self.kpts2)
        )
        print(
    ...
    def test_essential_matrix_metric_simple(self):
        estimator = pose_estimator_factory(
            PoseEstimatorType.ESSENTIAL_MATRIX_METRIC_SIMPLE, self.K1, self.K2
        )
        R21, t21, inliers = estimator.estimate(
            PoseEstimatorInput(
                kpts1=self.kpts1, kpts2=self.kpts2, depth1=self.depth1, depth2=self.depth2
            )
        )
        print(
    ...
    def test_essential_matrix_metric(self):
        estimator = pose_estimator_factory(
            PoseEstimatorType.ESSENTIAL_MATRIX_METRIC, self.K1, self.K2
        )
        R21, t21, inliers = estimator.estimate(
            PoseEstimatorInput(
                kpts1=self.kpts1, kpts2=self.kpts2, depth1=self.depth1, depth2=self.depth2
    ...
    def test_pnp(self):
        ...
        R21, t21, inliers = estimator.estimate(
            PoseEstimatorInput(kpts1=self.kpts1, kpts2=self.kpts2, depth1=self.depth1)
        )
        print(
            f"\n[test_pnp]:\n R21 = {R21.flatten()},\n t21 = {t21.flatten()},\n #inliers = {inliers}"
        )
    ...
    def test_procrustes(self):
        ...
        self.depth2 = project_depth(
            self.K1, self.depth1, self.K2, self.Rc2c1, self.tc2c1, self.w2, self.h2
        )  # for gettin a meaningful ICP refinement
        R21, t21, inliers = estimator.estimate(
            PoseEstimatorInput(
                kpts1=self.kpts1, kpts2=self.kpts2, depth1=self.depth1, depth2=self.depth2
            )
