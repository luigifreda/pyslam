...
from pyslam.utilities.img_management import img_blocks
from pyslam.utilities.features import nms_from_map, get_best_score_idxs, get_best_points_coordinates


shape = (500, 500)
im = np.abs(np.random.normal(loc=0, scale=255.0 / 3, size=shape)).astype(np.uint8)
cv2.imshow("image", im)
