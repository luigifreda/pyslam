...
class LoopCloserBase:
    ...
    def update_similarity_matrix_and_loop_closure_imgs(
        self, score, entry_id, img_id, other_entry_id, other_frame_id
    ):
...
class LoopCloserDBoW3(LoopCloserBase):
    ...
    def db_query(self, local_des: np.ndarray, img_id, max_num_results=5):
        results = self.db.query(
            local_des, max_results=max_num_results + 1
        )  # we need plus one to eliminate the best trivial equal to img_id
        return results
        ...
        if self.entry_id >= 1:
            if use_local_des:
                results = self.db_query(
                    des, img_id, max_num_results=kMaxResultsForLoopClosure + 1
                )  # we need plus one to eliminate the best trivial equal to img_id
            else:
                results = self.db_query(
            ...
            for r in results:
                ...
                self.update_similarity_matrix_and_loop_closure_imgs(
                    score=r.score,
