...
class GlobalFeatureDatabase:
    ...
    def get_max_score(self, g_des, img_id, max_num_results=5):
        ...
        if self.global_descriptor_type == "SAD":
            self.S_color[: global_des_database.shape[0], : global_des_database.shape[0]] = (
                convert_float_to_colored_uint8_image(
                    self.S_float[: global_des_database.shape[0], : global_des_database.shape[0]]
                )
            )
        else:
...
if __name__ == "__main__":
    ...
    while dataset.is_ok:
        ...
        if img is not None:
            ...
            global_des_database.add(
                g_des
            )  # add image descriptors to database after having computed the scores

            if entry_id > 1:
                best_idxs, best_scores = global_des_database.get_max_score(
                    g_des, img_id, max_num_results=kMaxResultsForLoopClosure + 1
                )  # we need plus one since we eliminate the best trivial equal to img_id
