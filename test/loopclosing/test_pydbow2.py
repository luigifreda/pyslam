    ...
    while dataset.is_ok:
        ...
        if img is not None:
            ...
            global_des_database.add(
                g_des
            )  # add image descriptors to global_des_database after having computed the scores

            if entry_id >= 1:
                best_idxs, best_scores = global_des_database.get_max_score(
                    g_des, img_id, max_num_results=kMaxResultsForLoopClosure
                )
