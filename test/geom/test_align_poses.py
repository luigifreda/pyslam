...
class TestAlignSVD:
    ...
    def test_align_svd(self):
        ...
        assoc_timestamps, assoc_filter_poses, assoc_gt_poses = find_poses_associations(
            self.filter_t_s, self.filter_T_wi, self.gt_t_s, self.gt_T_wi
        )
        num_assoc_samples = len(assoc_timestamps)
        assoc_t_wi = np.array(
            [assoc_filter_poses[i][0:3, 3] for i in range(num_assoc_samples)]
        ).reshape(num_assoc_samples, 3)
        assoc_gt_t_wi = np.array(
            [assoc_gt_poses[i][0:3, 3] for i in range(num_assoc_samples)]
        ).reshape(num_assoc_samples, 3)
        ...
        T_gt_est, rms_error, alignment_data = align_trajectories_with_svd(
            assoc_timestamps,
            assoc_t_wi,
            assoc_timestamps,
            assoc_gt_t_wi,
        ...
        ape_stats, T_evo_gt_est = evaluate_evo(
            poses_est=assoc_filter_poses,
            poses_gt=assoc_gt_poses,
            is_monocular=self.find_scale,
            plot_dir=None,
            label=None,
            save_metrics=False,
            save_plot=False,
        ...
        if align_to_gt:
            ...
            self.aligned_T_wi = [
                T_gt_est @ self.filter_T_wi[i] for i in range(self.num_filter_samples)
            ]
        else:
        ...
        print(
            f"spatial noise mean: {self.spatial_noise_mean}, spatial noise std: {self.spatial_noise_std}"
        )
        ...
        assoc_filter_poses_aligned = [
            T_gt_est @ assoc_filter_poses[i] for i in range(num_assoc_samples)
        ]
        errors = [
            assoc_filter_poses_aligned[i][:3, 3] - assoc_gt_poses[i][:3, 3]
            for i in range(num_assoc_samples)
        ]
        ...
        poses_est_evo_aligned = [
            T_evo_gt_est @ assoc_filter_poses[i] for i in range(num_assoc_samples)
        ]
        evo_errors = [
            poses_est_evo_aligned[i][:3, 3] - assoc_gt_poses[i][:3, 3]
            for i in range(num_assoc_samples)
        ]
        ...
        viewer3d.draw_cameras(
            [self.gt_T_wi, self.aligned_T_wi, poses_est_evo_aligned],
            [[1, 0, 0], [0, 1, 0], [0, 0, 1]],
        )
