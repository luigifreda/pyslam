...
images_paths = [
    ...
    kMast3rFolder
    + "/assets/NLE_tower/2679C386-1DC0-4443-81B5-93D7EDE4AB37-83120-000041DADB2EA917.jpg",
]
...
if __name__ == "__main__":
    ...
    if not are_scales_equal:
        ...
        depth[valid_first] = pts3d[0][
            valid_first, 2
        ]  # extract the z-component from the point cloud
        ...
        depth_map = point_cloud_to_depth(
            global_pc.vertices.reshape(-1, 3), intrinsics_original, w_original, h_original
        )
        depth_map_img = img_from_floats(depth_map)
    ...
    if show_image_tables:
        ...
        for i, conf in enumerate(confs):
            print(
                f"adding confidence image {i}, shape: {conf.shape}, min: {np.min(conf)}, max: {np.max(conf)}"
            )
