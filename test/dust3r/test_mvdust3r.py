...
def get_3D_dense_map(output, min_conf_thr=3, as_pointcloud=True): #, transparent_cams=False, cam_size=0.05, only_model=False):
    """
    ...
    with torch.no_grad():
        ...
        focals = torch.Tensor([focals]).reshape(1,).repeat(len(rgb_imgs))
...
def get_args_parser():
    ...
    parser_weights.add_argument("--weights", type=str, help="path to the model weights", default=kMvdust3rFolder + "/checkpoints/MVD.pth")
    parser_weights.add_argument("--model_name", type=str, help="name of the model weights", choices=["MVD", "MVDp"])
...
if __name__ == '__main__':    
    ...
    if args.model_name == "MVD":
        ...
        model_loaded = AsymmetricCroCo3DStereoMultiView.from_pretrained(weights_path).to(args.device)
