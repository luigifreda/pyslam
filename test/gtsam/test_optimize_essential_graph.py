...
class DataGenerator:
    ...
    def generate_loop_data(self):
        ...
        for i in range(n):
            ...
            x2d_n += current_scale * velocity * np.cos(theta_n) + abs(random.gauss(0, sigma_noise_xyz)) 
            y2d_n += current_scale * velocity * np.sin(theta_n) + abs(random.gauss(0, sigma_noise_xyz))
    ...
    def add_loop_closure(self): 
      ...
      # add connections among last_keyframe and [first_keyframe + its connected keyframes]
        if True:
            ...
            print(f'first keyframe: {first_kf.id}, connected keyframes: {[k.id for k in first_connected_keyframes]}')
        ...
        for connected_kfi, corrected_Siw in self.corrected_sim3_map.items():
            ...
            print(f'correcting keyframe: {connected_kfi.id}, current_Rwi_rpy: {rpy_from_rotation_matrix(corrected_Swi.R)}, corrected_tiw: {corrected_Tiw[:3,3].flatten()}')
...
def main():
    ...
    data_generator = DataGenerator(n=50, radius=5.0, sigma_noise_xyz=0.02, sigma_noise_theta_deg=0.1)
    data_generator.generate_loop_data()
    
    min_delta_ang, max_delta_ang = np.pi/4, np.pi/4
    min_delta_lin, max_delta_lin = 1.0, 1.0
    random_yaw, random_pitch, random_roll = np.random.uniform(min_delta_ang, max_delta_ang), np.random.uniform(min_delta_ang, max_delta_ang), np.random.uniform(min_delta_ang, max_delta_ang)
    random_tx, random_ty, random_tz = np.random.uniform(min_delta_lin, max_delta_lin), np.random.uniform(min_delta_lin, max_delta_lin), np.random.uniform(min_delta_lin, max_delta_lin)
    R_rand = rotation_matrix_from_yaw_pitch_roll(np.rad2deg(random_yaw), np.rad2deg(random_pitch), np.rad2deg(random_roll)) #rotation_matrix_from_yaw_pitch_roll(random_yaw, random_pitch, random_roll)
    ...
    print(f'loop_keyframe: {data_generator.loop_keyframe.id if data_generator.loop_keyframe else None}')
    ...
    for kf in data_generator.loop_connections.keys():
        print(f'keyframe: {kf.id}, connected_keyframes: {[c_kf.id for c_kf in data_generator.loop_connections[kf]]}')
    print(f'corrected_sim3_map: {[kf.id for kf in data_generator.corrected_sim3_map.keys()]}') 
    print(f'non_corrected_sim3_map: {[kf.id for kf in data_generator.non_corrected_sim3_map.keys()]}')
    ...
    if True:
        ...
        viewer3d.draw_cameras([data_generator.gt_poses, est_poses, est_poses_before], [[1,0,0], [0,1,0], [0,0,1]], show_trajectory_line=True)
    else:
