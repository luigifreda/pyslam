...
Sim3PointRegistrationSolver::Sim3PointRegistrationSolver(const Sim3PointRegistrationSolverInput& input):
...
void Sim3PointRegistrationSolver::SetRansacParameters(double probability, int minInliers, int maxIterations)
{
...
Eigen::Matrix4f Sim3PointRegistrationSolver::iterate(const int nIterations, bool &bNoMore, vector<uint8_t> &vbInliers, int &nInliers, bool& bConverged)
{
    bNoMore = false;
    bConverged = false;
    ...
    {
        if(kVerbose)
            std::cout << "[Sim3PointRegistrationSolver] Iteration " << mnIterations << " of " << nIterations << std::endl;        
        nCurrentIterations++;
        mnIterations++;
...
Eigen::Matrix4f Sim3PointRegistrationSolver::find(vector<uint8_t> &vbInliers12, int &nInliers, bool& bConverged)
{
    bool bFlag;
    return iterate(mRansacMaxIts,bFlag,vbInliers12,nInliers,bConverged);
}

void Sim3PointRegistrationSolver::ComputeCentroid(Eigen::Matrix3f &P, Eigen::Matrix3f &Pr, Eigen::Vector3f &C)
{
    C = P.rowwise().sum();
    C = C / P.cols();
...
Eigen::Matrix4f Sim3PointRegistrationSolver::GetEstimatedTransformation()
{
    return mBestT12;
}

Eigen::Matrix3f Sim3PointRegistrationSolver::GetEstimatedRotation()
{
    return mBestRotation;
}

Eigen::Vector3f Sim3PointRegistrationSolver::GetEstimatedTranslation()
{
    return mBestTranslation;
}

float Sim3PointRegistrationSolver::GetEstimatedScale()
{
    return mBestScale;
}
