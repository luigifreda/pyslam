...
Eigen::Matrix4f Sim3Solver::iterate(const int nIterations, bool &bNoMore, vector<uint8_t> &vbInliers, int &nInliers, bool& bConverged)
{
    bNoMore = false;
    bConverged = false;
    ...
    {
        if(kVerbose)
            std::cout << "[Sim3Solver] Iteration " << mnIterations << " of " << nIterations << std::endl;        
        nCurrentIterations++;
        mnIterations++;
...
void Sim3Solver::Project(const vector<Eigen::Vector3f> &vP3Dw, const Eigen::Matrix4f& Tcw, const Eigen::Matrix3f& K, vector<Eigen::Vector2f> &vP2D, std::vector<float>& depths)
{
    ...
    for(size_t i=0, iend=vP3Dw.size(); i<iend; i++)
    {
        ...
        const Eigen::Vector2f pt2D = Eigen::Vector2f(fx*P3Dc(0)*invz+cx, fy*P3Dc(1)*invz+cy);     
        vP2D.push_back(pt2D);
        depths.push_back(P3Dc(2));
    }
}

void Sim3Solver::FromCameraToImage(const vector<Eigen::Vector3f> &vP3Dc, const Eigen::Matrix3f& K, vector<Eigen::Vector2f> &vP2D)
{
    const float &fx = K(0,0);
    const float &fy = K(1,1);
    const float &cx = K(0,2);
    ...
    for(size_t i=0, iend=vP3Dc.size(); i<iend; i++)
    {
        ...
        const Eigen::Vector2f pt2D = Eigen::Vector2f(fx*vP3Dc[i](0)*invz+cx, fy*vP3Dc[i](1)*invz+cy);
        vP2D.push_back(pt2D);
    }
}
