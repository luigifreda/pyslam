...
void extractPatch(const cv::Mat &image, const cv::KeyPoint &kp, const int &patch_size,
                  cv::Mat &patch, const bool use_orientation = true, const float scale_factor = 1.0,
                  const int warp_flags = cv::WARP_INVERSE_MAP + cv::INTER_CUBIC +
...
void extractPatches(const cv::Mat &image, const std::vector<cv::KeyPoint> &kps,
                    ...
                    const bool use_orientation = true, const float scale_factor = 1.0,
                    const int warp_flags = cv::WARP_INVERSE_MAP + cv::INTER_CUBIC +
                                           cv::WARP_FILL_OUTLIERS);

std::pair<py::array_t<int>, py::array_t<int>>
goodMatchesSimple(const std::vector<std::pair<cv::DMatch, cv::DMatch>> &matches,
                  float ratio_test = 0.7f);
