...
Beyond SLAM, pySLAM also provides a unified \textit{ScenefromViews} architecture for modern multi-view 3D reconstruction starting from a collection of images.
...
\begin{itemize}
    ...
    the scene, such as DeepLabv3, Segformer, CLIP, EOV-Seg, Detic, and ODISE.
    
    \item A \textbf{\protect\hyperlink{scene-from-views}{Scene from Views}} module providing a unified interface for 3D scene reconstruction from multiple views, supporting models such as DUSt3R, MASt3R, Depth Anything V3, MV-DUSt3R, and VGGT.
    
    \item A \textbf{\protect\hyperlink{cpp-core}{C++ Core}} module for high-performance sparse SLAM, with pybind11 bindings enabling zero-copy data exchange between Python and C++.
...
\item
  ...
  \item
  \texttt{main\_scene\_from\_views.py} demonstrates how to use the Scene from Views
  factory for 3D scene reconstruction from multiple views using various
  reconstruction models (see
...
The diagram in Fig.~\ref{Fig:VolumetricIntegrator} illustrates the structure of the \textit{Volumetric Integrator} module. At its core, the \texttt{volumetric\_integrator\_factory} generates specific volumetric integrator instances based on the selected \texttt{volumetric\_integrator\_type}, such as \texttt{VOXEL\_GRID}, \texttt{VOXEL\_SEMANTIC\_GRID}, \texttt{VOXEL\_SEMANTIC\_PROBABILISTIC\_GRID}, \texttt{TSDF}, and \texttt{GAUSSIAN\_SPLATTING}.
...
Both \texttt{SemanticMappingDense} and \texttt{SemanticMappingDenseProcess} integrate semantic information into the SLAM map by leveraging per-frame predictions from a semantic segmentation model. The segmentation model is instantiated via the \texttt{semantic\_segmentation\_factory}, based on the selected \texttt{semantic\_segmentation\_type}. Supported segmentation backends include \texttt{DEEPLABV3}, \texttt{SEGFORMER}, \texttt{CLIP}, \texttt{EOV\_SEG}, \texttt{DETIC}, and \texttt{ODISE}, each of which corresponds to a dedicated class (\texttt{SemanticSegmentationDeepLabV3}, \texttt{SemanticSegmentationSegformer}, \texttt{SemanticSegmentationCLIP}, \texttt{SemanticSegmentationEovSeg}, \texttt{SemanticSegmentationDetic}, \texttt{SemanticSegmentationOdise}) inheriting from the shared \texttt{SemanticSegmentationBase}.
...
The diagram in Fig.~\ref{Fig:SceneFromViews} illustrates the architecture of the \textit{Scene from Views} module, which provides a unified interface for 3D scene reconstruction from multiple views. At its core, the \texttt{scene\_from\_views\_factory} instantiates specific reconstruction models based on the selected \texttt{SceneFromViewsType}, such as \texttt{DUST3R}, \texttt{MAST3R}, \texttt{DEPTH\_ANYTHING\_V3}, \texttt{MVDUST3R}, \texttt{VGGT}, and \texttt{VGGT\_ROBUST}.

Each type creates a corresponding implementation (e.g., \texttt{SceneFromViewsDust3r}, \texttt{SceneFromViewsMast3r}, \texttt{SceneFromViewsDepthAnythingV3}, \texttt{SceneFromViewsMvdust3r}, \texttt{SceneFromViewsVggt}, \texttt{SceneFromViewsVggtRobust}), all inheriting from a common \texttt{SceneFromViewsBase}. This base class implements a unified three-step reconstruction pipeline: \texttt{preprocess\_images()} prepares input images for the specific model, \texttt{infer()} runs model inference, and \texttt{postprocess\_results()} converts raw model output to a standardized \texttt{SceneFromViewsResult} format containing merged point clouds, meshes, camera poses, and optional depth maps or intrinsics.
...
\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/config_parameters.py}{pyslam/config\_parameters.py}. Further
...
\texttt{pyslam/config\_parameters.py}. At present, the following methods are available:
\texttt{VOXEL\_GRID}, \texttt{VOXEL\_SEMANTIC\_GRID}, \texttt{VOXEL\_SEMANTIC\_PROBABILISTIC\_GRID}, \texttt{TSDF}, and \texttt{GAUSSIAN\_SPLATTING} (see
\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/dense/volumetric_integrator_factory.py}{pyslam/dense/volumetric\_integrator\_factory.py}). Note that you need CUDA
...
image center, computed using the median depth of the frame. With this
policy, a new keyframe is generated only if its FOV center lies beyond
a predefined distance from the nearest existing keyframe's FOV
...
\texttt{kDoSparseSemanticMappingAndSegmentation=True} in \texttt{config\_parameters.py}. The
...
The \textit{Scene from Views} module provides a unified interface for 3D scene reconstruction from multiple views. It follows a modular architecture that allows easy integration of different reconstruction models while maintaining a consistent API.

The module is built around a factory pattern with a base class \texttt{SceneFromViewsBase} that implements a unified three-step reconstruction pipeline:
\begin{enumerate}
\item \texttt{preprocess\_images()}: Preprocesses input images for the specific model
\item \texttt{infer()}: Runs model inference on preprocessed images
\item \texttt{postprocess\_results()}: Converts raw model output to standardized \texttt{SceneFromViewsResult}
\end{enumerate}

The \texttt{SceneFromViewsResult} structure standardizes the output format across all models, containing:
\begin{itemize}
\item \texttt{global\_point\_cloud}: Merged point cloud in world coordinates
\item \texttt{global\_mesh}: Merged mesh in world coordinates
\item \texttt{camera\_poses}: List of camera-to-world transformation matrices (4x4)
\item \texttt{processed\_images}: List of processed images used by the model
\item \texttt{depth\_predictions}: List of depth maps if available
\item \texttt{point\_clouds}: List of per-view point clouds
\item \texttt{intrinsics}: List of camera intrinsic matrices if available
\item \texttt{confidences}: List of confidence maps if available
\end{itemize}

The following reconstruction models are currently supported:

\begin{itemize}
\item \texttt{DUST3R}: Dense 3D reconstruction from arbitrary image collections without requiring known camera intrinsics or poses. Predicts dense 3D pointmaps for image pairs, from which depth maps and camera parameters can be recovered. Supports global alignment optimization to merge pairwise reconstructions.
\item \texttt{MAST3R}: Builds on DUSt3R by adding a 3D-aware dense matching head. Produces both dense 3D pointmaps and dense local features for highly accurate matching, with sparse global alignment and optional TSDF-based fusion.
\item \texttt{DEPTH\_ANYTHING\_V3}: Monocular depth estimation model that can produce both relative and metric depth. Supports optional camera pose and intrinsic parameter estimation for reconstruction pipelines.
\item \texttt{MVDUST3R}: Multi-view variant of DUSt3R that processes multiple unposed RGB views in a single feed-forward pass, using multi-view decoder blocks to jointly reason about all input views.
\item \texttt{VGGT}: Large feed-forward transformer for 3D reconstruction that directly predicts camera parameters, pointmaps, depth maps, and 3D point tracks from one or many input views in a single forward pass.
\item \texttt{VGGT\_ROBUST}: VGGT variant mirroring the \texttt{robust\_vggt} demo. It scores each view against an anchor frame using global attention maps and cosine similarity, rejects low-scoring views before the final forward pass, and returns metadata about dropped/kept views (\texttt{rejected\_indices}, \texttt{survivor\_indices}). Configurable knobs include anchor weighting (\texttt{attn\_a}, \texttt{cos\_a}), rejection threshold (\texttt{rej\_thresh}), anchor choice (\texttt{use\_most\_central\_as\_reference}), and masking/confidence filters. Default checkpoint: \texttt{facebook/VGGT-1B}.
\end{itemize}

The module supports both pairwise models (DUSt3R, MASt3R) that process image pairs and perform global alignment, as well as multi-view models (MV-DUSt3R, VGGT) that process multiple views simultaneously. Usage examples are available in the script \texttt{main\_scene\_from\_views.py}.
...
\begin{itemize}
\tightlist
\item A C++ template-based voxel grid implementation leverages parallel spatial hashing, supporting both direct voxel hashing and indirect voxel-block hashing strategies. Parallel execution is managed using TBB, and the design accommodates both simple and semantic voxels. Further information about the volumetric grid models is available \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/cpp/volumetric/README.md}{here}.
\item
