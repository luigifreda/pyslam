...
\begin{itemize}
    ...
    the scene, such as DeepLabv3\cite{chen2017rethinking}, Segformer\cite{xie2021segformer}, CLIP\cite{lin2023clip}, EOV-Seg\cite{eov_seg}, DETIC\cite{detic}, and ODISE\cite{odise}.
    
    \item A \textbf{\protect\hyperlink{scene-from-views}{Scene from Views}} module providing a unified interface for end-to-end inference of 3D scenes from multiple views, supporting models such as DUSt3R\cite{wang2023dust3r}, MASt3R\cite{leroy2024grounding}, Depth Anything V3\cite{depthanything3}, MV-DUSt3R\cite{tang2025mv}, and VGGT\cite{wang2025vggt}.
...
\item
  ...
  images using models like Mast3r, VGGT, and DepthFromAnythingV3 (see
...
The module supports both pairwise models (\texttt{DUSt3R}\cite{wang2023dust3r}, \texttt{MASt3R}\cite{leroy2024grounding}) that process image pairs and perform global alignment, as well as multi-view models (\texttt{DEPTH\_ANYTHING\_V3}\cite{depthanything3}, \texttt{MV-DUSt3R}\cite{tang2025mv}, \texttt{VGGT}\cite{wang2025vggt}, \texttt{Robust VGGT}\cite{vggt_robust}) that process multiple views simultaneously in a single forward pass. This modular design enables flexible integration of diverse 3D reconstruction techniques while maintaining a consistent API across different model architectures.
...
The \textit{Scene from Views} module provides a unified interface for end-to-end 3D scene reconstruction from multiple
...
\tightlist
\item
  End-to-end multi-view reconstruction (poses + fused 3D geometry
  directly from images): \texttt{SceneFromViewsDust3r},
  \texttt{SceneFromViewsMast3r}, \texttt{SceneFromViewsMvdust3r},
  \texttt{SceneFromViewsVggt}, \texttt{SceneFromViewsVggtRobust}.
\item
...
take two images at a time. Multi-view end-to-end reconstruction is
achieved by running them on many image pairs and performing a global
alignment / optimization over all pairwise pointmaps.
...
\end{itemize}
\hypertarget{supported-models-for-end-to-end-inference-of-3d-scenes-from-multiple-images}{%
\subsubsection{Supported models for end-to-end inference of 3D scenes
from multiple
images}\label{supported-models-for-end-to-end-inference-of-3d-scenes-from-multiple-images}}
