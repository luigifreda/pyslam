...
class VolumetricIntegratorVoxelGrid(VolumetricIntegratorBase):
    ...
    def init(
        ...
        if use_voxel_blocks:
            VolumetricIntegratorBase.print(f"VolumetricIntegratorVoxelGrid: using voxel blocks")
            self.volume = VoxelBlockGrid(
    ...
    def volume_integration(
        ...
        # print('VolumetricIntegratorVoxelGrid: volume_integration')
        last_output = None
        do_output = False
        ...
        try:
            if is_running.value == 1:
                ...
                if self.last_input_task is None:
                    ...
                    if self.last_input_task.task_type == VolumetricIntegrationTaskType.INTEGRATE:
                        ...
                        # print(f"VolumetricIntegratorVoxelGrid: color_undistorted: shape: {color_undistorted.shape}, type: {color_undistorted.dtype}")
                    ...
                    if do_output:
                        ...
                        VolumetricIntegratorBase.print(
                            f"VolumetricIntegratorVoxelGrid: id: {self.last_integrated_id} -> PointCloud: points: {pc_out.points.shape}"
                        )
                        ...
                        VolumetricIntegratorBase.print(
                            f"VolumetricIntegratorVoxelGrid: last output id: {last_output.id if last_output is not None else None}"
                        )
                    ...
                    if is_running.value == 1 and last_output is not None:
                        if (
                            ...
                            with q_out_condition:
                                ...
                                VolumetricIntegratorBase.print(
                                    f"VolumetricIntegratorVoxelGrid: pushed new output to q_out size: {q_out.qsize()}"
                                )
            ...
            VolumetricIntegratorBase.print(f"VolumetricIntegratorVoxelGrid: EXCEPTION: {e} !!!")
        ...
        VolumetricIntegratorBase.print(
            f"VolumetricIntegratorVoxelGrid: {id_info}, last integrated id: {self.last_integrated_id}, q_in size: {q_in.qsize()}, q_out size: {q_out.qsize()}, volume-integration elapsed time: {time_volumetric_integration.value}"
        )
