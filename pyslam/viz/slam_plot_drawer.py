    ...
    def draw(self, img_id):
        try:
            ...
            if self.matched_points_plt is not None:
                ...
                if self.slam.tracking.num_matched_map_points is not None:
                    valid_matched_map_points_signal = [
                        img_id,
                        self.slam.tracking.num_matched_map_points,
                ...
                if self.slam.tracking.num_kf_ref_tracked_points is not None:
                    kf_ref_tracked_points_signal = [
                        img_id,
                        self.slam.tracking.num_kf_ref_tracked_points,
                    ]
                    self.matched_points_plt.draw(
            ...
            if self.info_3dpoints_plt is not None:
                if (
                    self.slam.local_mapping.last_processed_kf_img_id is not None
                    and self.last_processed_kf_img_id
                    != self.slam.local_mapping.last_processed_kf_img_id
                ):
                    ...
                    if self.slam.tracking.last_num_static_stereo_map_points is not None:
                        num_static_stereo_map_points_signal = [
                            img_id,
                            self.slam.tracking.last_num_static_stereo_map_points,
                        ]
                        self.info_3dpoints_plt.draw(
                            num_static_stereo_map_points_signal,
            ...
            if self.chi2_error_plt is not None:
                if self.slam.tracking.mean_pose_opt_chi2_error is not None:
                    mean_pose_opt_chi2_error_signal = [
                        img_id,
                        self.slam.tracking.mean_pose_opt_chi2_error,
                    ]
                ...
                if self.slam.local_mapping.mean_ba_chi2_error is not None:
                    mean_squared_reproj_error_signal = [
                        img_id,
                        self.slam.local_mapping.mean_ba_chi2_error,
                    ]
                    self.chi2_error_plt.draw(
                ...
                if self.slam.loop_closing is not None:
                    if self.slam.loop_closing.mean_graph_chi2_error is not None:
                        mean_graph_chi2_error_signal = [
                            img_id,
                            self.slam.loop_closing.mean_graph_chi2_error,
                        ]
                        self.chi2_error_plt.draw(
            ...
            if self.timing_plt is not None:
                ...
                if self.slam.local_mapping.timer_triangulation.last_elapsed:
                    time_local_mapping_triangulation_signal = [
                        img_id,
                        self.slam.local_mapping.timer_triangulation.last_elapsed,
                    ]
                ...
                if self.slam.local_mapping.timer_pts_culling.last_elapsed:
                    time_local_mapping_pts_culling_signal = [
                        img_id,
                        self.slam.local_mapping.timer_pts_culling.last_elapsed,
                    ]
                    self.timing_plt.draw(
                        time_local_mapping_pts_culling_signal,
                ...
                if self.slam.local_mapping.timer_kf_culling.last_elapsed:
                    time_local_mapping_kf_culling_signal = [
                        img_id,
                        self.slam.local_mapping.timer_kf_culling.last_elapsed,
                    ]
                    self.timing_plt.draw(
                ...
                if self.slam.local_mapping.timer_pts_fusion.last_elapsed:
                    time_local_mapping_pts_fusion_signal = [
                        img_id,
                        self.slam.local_mapping.timer_pts_fusion.last_elapsed,
                    ]
                    self.timing_plt.draw(
                ...
                if self.slam.loop_closing is not None:
                    if self.slam.loop_closing.timer.last_elapsed:
                        time_loop_closing_signal = [
                            img_id,
                            self.slam.loop_closing.timer.last_elapsed,
                        ]
                        self.timing_plt.draw(
                            time_loop_closing_signal,
                            "loop closing",
                    ...
                    if self.slam.loop_closing.time_loop_detection.value:
                        time_loop_detection_signal = [
                            img_id,
                            self.slam.loop_closing.time_loop_detection.value,
                        ]
                        self.timing_plt.draw(
                            time_loop_detection_signal,
                            "loop detection",
                ...
                if self.slam.volumetric_integrator is not None:
                    if self.slam.volumetric_integrator.time_volumetric_integration.value:
                        time_volumetric_integration_signal = [
                            img_id,
                            self.slam.volumetric_integrator.time_volumetric_integration.value,
                        ]
                        self.timing_plt.draw(
                            time_volumetric_integration_signal,
            ...
            if self.traj_error_plt is not None and new_alignment_data is not None:
                ...
                print(
                    f"SlamPlotDrawer: new gt alignment timestamp: {new_alignment_timestamp}, rms_error: {new_alignment_data.rms_error}, max_error: {new_alignment_data.max_error}, is_est_aligned: {new_alignment_data.is_est_aligned}"
                )
                if (
                    ...
                    if False:
                        ...
                        if False:
                            ...
                            print(
                                f"SlamPlotDrawer: err_x_max: {err_x_max}, err_y_max: {err_y_max}, err_z_max: {err_z_max}"
                            )
