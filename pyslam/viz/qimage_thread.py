...
class QimageViewer:
    ...
    def init_window(self, name, width, height):
        ...
        resizer = Resizer(window, name, self.max_width, self.max_height, self.screen_width, self.screen_height)
    ...
    def update_image(self, image, name, format=QImage.Format_BGR888):
        ...
        is_label_size_different = (abs(label_size.width() - window_size.width()) > 1e-3 or \
                                   abs(label_size.height() - window_size.height()) > 1e-3)
        if abs(resizer.aspect_ratio - new_aspect_ratio) > 1e-3 or is_label_size_different: 
            if kVerbose:
                print(f'QimageViewer: window {name}:  image size: {width}x{height}, aspect ratio changed from {resizer.aspect_ratio} to {new_aspect_ratio}')
