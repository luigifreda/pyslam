...
from pyslam.config_parameters import Parameters # TODO(dvdmc): do we want parameters to be used in this file?
import g2o

from pyslam.semantics.semantic_mapping_shared import SemanticMappingShared # TODO(dvdmc): do we want semnatics to be used in this file?
...
def pose_optimization(frame, verbose=False, rounds=10):
    ...
    if draw_chi2_histograms:
        ...
        if chi2_mono_vals:
            hist_img_mono = draw_histogram(chi2_mono_vals, bins=10, delta=chi2Mono, min_value=0, max_value=chi2Mono*10, color=(255, 0, 0))
            cv2.imshow('Monocular chi2 errors', hist_img_mono)
        if chi2_stereo_vals:
            hist_img_stereo = draw_histogram(chi2_stereo_vals, bins=10, delta=chi2Stereo, min_value=0, max_value=chi2Stereo*10, color=(0, 255, 0))
...
def local_bundle_adjustment(keyframes: list[KeyFrame], points: list[MapPoint], keyframes_ref: list[KeyFrame] = [], fixed_points: bool = False, verbose: bool = False, rounds: int = 10, abort_flag: g2o.Flag = g2o.Flag(), mp_abort_flag = None, map_lock = None):
...
def lba_optimization_process(result_dict_queue, queue, good_keyframes, keyframes_ref, good_points, fixed_points, verbose, rounds, mp_abort_flag):
    ...
    try:
        ...
        keyframe_poses = {kf.kid: poseRt(graph_keyframes[kf].estimate().rotation().matrix(), graph_keyframes[kf].estimate().translation()) for kf in graph_keyframes}
...
def optimize_sim3(kf1: KeyFrame, kf2: KeyFrame,
    ...
    for i, edge_12 in enumerate(edges_12):
        ...
        if edge_12.chi2() > th2 or not edge_12.is_depth_positive() or\
           edge_21.chi2() > th2 or not edge_21.is_depth_positive():
