...
def epiline_to_end_points(line, e, xinf, cols):
    ...
    if (
        not math.isnan(xmin[0]) and not math.isnan(xmin[1]) and xmin[0] > u0 and xmin[0] < cols
    ):  # xmin is on the right of (u0,v0) => replace (u0,v0)
        u0 = xmin[0]
        v0 = xmin[1]

    if (
        not math.isnan(xmax[0]) and not math.isnan(xmax[1]) and xmax[0] < u1 and xmax[0] > 0
...
# NOTE: this is an experimental version, just for testing
def _search_frame_for_triangulation(f1: Frame, f2: Frame, img2, img1=None):
    ...
    line_edges = [
        epiline_to_end_points(line, e2, x2_inf, f2.width) for line, x2_inf in zip(lines2, xs2_inf)
    ]
