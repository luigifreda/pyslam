...
std::string Frame::to_json() const {
    ...
    try {
        ...
        if (kps.size() > 0) {
            std::vector<std::vector<double>> kps_list;
        ...
        // Right keypoints
        if (kps_r.size() > 0) {
            std::vector<std::vector<double>> kps_r_list;
        ...
        if (kpsu.size() > 0) {
            std::vector<std::vector<double>> kpsu_list;
        ...
        if (kpsn.size() > 0) {
            std::vector<std::vector<double>> kpsn_list;
...
Frame Frame::from_json(const std::string &json_str) {
    ...
    if (j.contains("pose") && !j["pose"].is_null()) {
        try {
            ...
            if (pose_array.size() == 4 && pose_array[0].size() == 4) {
                ...
                pose = new CameraPose(Eigen::Isometry3d(pose_matrix));
            } else {
    ...
    Eigen::Vector3d fov_center_c = safe_parse_vector3d(j, "fov_center_c");
    Eigen::Vector3d fov_center_w = safe_parse_vector3d(j, "fov_center_w");
    ...
    // Create the frame
    Frame frame(camera, cv::Mat(), cv::Mat(), cv::Mat(), pose, id, timestamp, img_id);
    ...
    frame.kps = safe_parse_keypoints(j, "kps");
    frame.kps_r = safe_parse_keypoints(j, "kps_r");
    frame.kpsu = safe_parse_keypoints(j, "kpsu");
    frame.kpsn = safe_parse_keypoints(j, "kpsn");
        ...
        try {
            ...
            for (size_t i = 0; i < point_ids.size(); ++i) {
                if (point_ids[i] != -1) {
                    ...
                    frame.points[i] =
                        reinterpret_cast<MapPoint *>(static_cast<intptr_t>(point_ids[i]));
                }
            }
        } catch (const std::exception &e) {
    ...
    if (kf_ref_id != -1) {
        ...
        frame.kf_ref = reinterpret_cast<KeyFrame *>(static_cast<intptr_t>(kf_ref_id));
    }
    ...
    if (frame.kps.size() > 0 && frame.points.size() > 0) {
        if (frame.kps.rows() != static_cast<int>(frame.points.size())) {
            throw std::runtime_error(
                "Keypoints and points size mismatch: " + std::to_string(frame.kps.rows()) + " vs " +
...
void Frame::replace_ids_with_objects(const std::vector<MapPoint *> &points,
    ...
    std::unordered_map<int, MapPoint *> points_dict;
    std::unordered_map<int, KeyFrame *> keyframes_dict;

    // Build points lookup dictionary
    for (const auto *point : points) {
        if (point != nullptr) {
            points_dict[point->id] = const_cast<MapPoint *>(point);
        }
    }
    ...
    for (const auto *keyframe : keyframes) {
        if (keyframe != nullptr) {
            keyframes_dict[keyframe->id] = const_cast<KeyFrame *>(keyframe);
        }
    }
    ...
    if (!this->points.empty()) {
        ...
        for (size_t i = 0; i < this->points.size(); ++i) {
            if (this->points[i] != nullptr) {
                ...
                int point_id = static_cast<int>(reinterpret_cast<intptr_t>(this->points[i]));
    ...
    if (kf_ref != nullptr) {
        // Extract the ID that was stored during deserialization
        int kf_ref_id = static_cast<int>(reinterpret_cast<intptr_t>(kf_ref));
...
py::tuple Frame::state_tuple() const {
    ...
    std::shared_ptr<KeyFrame> kfref_sp;
    if (kf_ref) {
        // Disambiguate shared_from_this through Frame base, then cast to KeyFrame
        kfref_sp = std::static_pointer_cast<KeyFrame>(kf_ref->Frame::shared_from_this());
    }
...
void Frame::restore_from_state(const py::tuple &t) {
    ...
    // ---- feature arrays ----
    kps = t[idx++].cast<MatNx2d>();
    kps_r = t[idx++].cast<MatNx2d>();
    kpsu = t[idx++].cast<MatNx2d>();
    kpsn = t[idx++].cast<MatNx2d>();
    ...
    // ---- map-point associations ----
    {
        auto mps = t[idx++].cast<std::vector<std::shared_ptr<MapPoint>>>();
        points.resize(mps.size(), nullptr);
        for (size_t i = 0; i < mps.size(); ++i) {
            points[i] = mps[i] ? mps[i].get() : nullptr;
        }
    }
    ...
    // ---- reference KF ----
    {
        auto kfref_sp = t[idx++].cast<std::shared_ptr<KeyFrame>>();
        kf_ref = kfref_sp ? kfref_sp.get() : nullptr;
    }
