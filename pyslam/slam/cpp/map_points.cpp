...
inline Eigen::Vector3d normalized_ray_in_world(const KeyFrame *kf, int idx) {
...
std::tuple<int, std::vector<bool>, std::vector<MapPoint *>>
Map::add_points(const std::vector<Eigen::Vector3d> &points3d,
                const std::optional<std::vector<bool>> &mask_pts3d_opt, KeyFrame *kf1,
                KeyFrame *kf2, const std::vector<int> &idxs1, const std::vector<int> &idxs2,
    ...
    std::vector<MapPoint *> added_points;
    ...
    if (do_check) {
        ...
        MSG_FORCED_ASSERT(camera1 != nullptr, "Map::add_points: camera1 is nullptr");
        const auto camera2 = kf2->camera;
        MSG_FORCED_ASSERT(camera2 != nullptr, "Map::add_points: camera2 is nullptr");

        for (size_t i = 0; i < num_points; ++i) {
            const int idx1 = idxs1[i];
            const int idx2 = idxs2[i];

            const auto [uv1, depth1] = kf1->project_point(points_world[i]);
            const auto [uv2, depth2] = kf2->project_point(points_world[i]);
            ...
            if (try_recover3d_from_stereo && is_stereo1 &&
                ...
                const auto [pt_world, ok] = kf1->unproject_point_3d(idx1, true);
                if (ok) {
                ...
                const auto [pt_world, ok] = kf2->unproject_point_3d(idx2, true);
                if (ok) {
                    points_world[i] = pt_world;
                    is_recovered3d_from_stereo = true;
                }
    ...
    for (size_t i = 0; i < num_points; ++i) {
        ...
        auto *mp = new MapPoint(points_world[i], color, kf2, idx2);
        this->add_point(mp);
...
int Map::add_stereo_points(const std::vector<Eigen::Vector3d> &points3d,
                           const std::optional<std::vector<bool>> &mask_pts3d_opt, Frame *f,
                           KeyFrame *kf, const std::vector<int> &idxs, const cv::Mat &img) {

    if (points3d.empty()) {
        return 0;
    }

    if (kf == nullptr || f == nullptr) {
    ...
    for (size_t i = 0; i < num_points; ++i) {
        ...
        auto *mp = new MapPoint(points3d[i], color, kf, idx);
        this->add_point(mp);
...
void Map::remove_points_with_big_reproj_err(const std::vector<MapPoint *> &points_to_check) {
    ...
    for (MapPoint *p : candidates) {
        ...
        for (const auto &[frame, idx] : observations) {
            if (frame == nullptr || idx < 0 || idx >= static_cast<int>(frame->kpsu.rows())) {
                continue;
            }

            const auto [proj, _] = frame->project_map_point(p);
...
float Map::compute_mean_reproj_error(const std::vector<MapPoint *> &points_subset) const {
    std::scoped_lock lock(_update_lock);
    ...
    const auto accumulate_point = [&](MapPoint *p) {
        if (p == nullptr || p->is_bad()) {
            return;
        }
