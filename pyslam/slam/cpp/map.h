...
class Map : public std::enable_shared_from_this<Map> {
  ...
  public:
    ...
    add_points(const std::vector<Eigen::Vector3d> &points3d,
               const std::optional<std::vector<bool>> &mask_pts3d, KeyFrame *kf1, KeyFrame *kf2,
               const std::vector<int> &idxs1, const std::vector<int> &idxs2, const cv::Mat &img,
               bool do_check = true, double cos_max_parallax = 0.9998,
               std::optional<double> far_points_threshold = std::nullopt);
    int add_stereo_points(const std::vector<Eigen::Vector3d> &points3d,
                          const std::optional<std::vector<bool>> &mask_pts3d, Frame *f,
