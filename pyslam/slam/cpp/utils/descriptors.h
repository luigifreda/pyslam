...
inline float descriptor_distance(const cv::Mat &a, const cv::Mat &b, NormType dist_type) noexcept {
    switch (dist_type) {
    case NormType::Hamming: {
        const int h = binary_descriptor_distance(a, b);
        return (h == INT_MAX) ? std::numeric_limits<float>::infinity() : static_cast<float>(h);
    }
    case NormType::L2:
