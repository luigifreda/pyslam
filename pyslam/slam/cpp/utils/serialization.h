...
inline Eigen::Vector3d safe_parse_vector3d(const nlohmann::json &j, const std::string &key) {
    try {
        if (j.contains(key) && !j[key].is_null()) {
            auto vec = j[key].get<std::vector<double>>();
            if (vec.size() == 3) {
...
inline MatNx2d safe_parse_keypoints(const nlohmann::json &j, const std::string &key) {
    try {
        if (j.contains(key) && !j[key].is_null()) {
            auto kps_array = j[key].get<std::vector<std::vector<double>>>();
            MatNx2d kps(kps_array.size(), 2);
