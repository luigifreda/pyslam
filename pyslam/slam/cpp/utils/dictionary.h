...
// Use a different approach - define ValueVariant without recursive types
// We'll use std::any for recursive types to break the circular dependency
using ValueVariant = std::variant<std::monostate, // null / unset
                                  bool,
                                  int, // Add int type
                                  int64_t, float, double, std::string,
                                  std::vector<double>, // Add vector<double> type
...
struct Value {
    ...
    Value(std::vector<double> v) : data(std::move(v)) {} // Add vector<double> constructor
    ...
    template <typename T> bool is() const {
        if constexpr (std::is_same_v<T, List>) {
            return data.index() == 8 && std::any_cast<List>(&std::get<std::any>(data)) != nullptr;
        } else if constexpr (std::is_same_v<T, Dict>) {
            return data.index() == 8 && std::any_cast<Dict>(&std::get<std::any>(data)) != nullptr;
        } else {
