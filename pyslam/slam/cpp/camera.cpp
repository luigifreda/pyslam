...
std::pair<pyslam::MatNx2d, pyslam::VecNd> CameraUtils::project_points(pyslam::MatNx3dRef xcs,
    ...
    // (fast pass + tiny fallback loop for invalids)
    auto bad = (z.array() <= 1e-6);
    if (bad.any()) {
        for (int i = 0; i < N; ++i) {
            if (bad(i)) {
                uv(i, 0) = -1.0;
...
std::pair<pyslam::MatNx3d, pyslam::VecNd>
CameraUtils::project_points_stereo(pyslam::MatNx3dRef xcs, pyslam::Mat3dRef K, const double bf) {
    const int N = static_cast<int>(xcs.rows());
    MatNx3d projections(N, 3);
    VecNd depths(N);
...
pyslam::MatNx2d CameraUtils::unproject_points(pyslam::MatNx2dRef uvs, pyslam::Mat3dRef Kinv) {
    const int N = static_cast<int>(uvs.rows());
    MatNx2d result(N, 2);

    for (int i = 0; i < N; ++i) {
        const Eigen::Vector3d uv_homogeneous(uvs(i, 0), uvs(i, 1), 1.0);
        const Eigen::Vector3d p = Kinv * uv_homogeneous;
        result(i, 0) = p.x();
        result(i, 1) = p.y();
    }
...
pyslam::MatNx3d CameraUtils::unproject_points_3d(pyslam::MatNx2dRef uvs, pyslam::Mat3dRef Kinv,
                                                 pyslam::VecNdRef depths) {
    const int N = static_cast<int>(uvs.rows());
    MatNx3d result(N, 3);

    for (int i = 0; i < N; ++i) {
        const Eigen::Vector3d uv_homogeneous(uvs(i, 0), uvs(i, 1), 1.0);
        const Eigen::Vector3d uv_scaled = uv_homogeneous * depths(i); // Scale by depth first
        const Eigen::Vector3d p = Kinv * uv_scaled; // Then apply inverse intrinsics
        result(i, 0) = p.x();
        result(i, 1) = p.y();
...
bool Camera::is_in_image(const Eigen::Vector2d &uv, double z) const {
...
Eigen::Vector2d Camera::unproject_point(const double u, const double v) const {
    const double x = (u - cx) / fx;
    const double y = (v - cy) / fy;
    return Eigen::Vector2d(x, y);
}

Eigen::Vector3d Camera::unproject_point_3d(const double u, const double v,
...
pyslam::MatNx2d PinholeCamera::undistort_points(pyslam::MatNx2dRef uvs) const {
    if (!is_distorted) {
        return uvs;
    }
    ...
    pyslam::MatNx2d result(uvs.rows(), 2);
    for (int i = 0; i < undistorted_uvs.rows; ++i) {
        result(i, 0) = undistorted_uvs.at<double>(i, 0);
        result(i, 1) = undistorted_uvs.at<double>(i, 1);
    }
    return result;
}
    ...
    try {
        ...
        if (camera_json.contains("camera_type")) {
            ...
            if (camera_type == "PinholeCamera") {
                return new PinholeCamera(PinholeCamera::from_json(camera_json.dump()));
            }
