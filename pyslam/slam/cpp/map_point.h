...
class MapPointBase {
  ...
  public:
    ...
    // Observations
    std::map<KeyFrame *, int> _observations;
    std::map<Frame *, int> _frame_views;
...
class MapPoint : public MapPointBase, public std::enable_shared_from_this<MapPoint> {
  ...
  public:
    ...
    // Update operations
    void update_normal_and_depth(Frame *frame = nullptr, int idxf = -1, bool force = false);
    ...
    static std::unique_ptr<MapPoint> from_json(const std::string &json_str);
...
inline float MapPoint::descriptor_distance(const cv::Mat &d1, const cv::Mat &d2) const {
