...
// cv::KeyPoint <-> (pt.x, pt.y, size, angle, response, octave)   (pybind11)
using KeyPointTuple = std::tuple<double, double, double, double, double, int>;
...
// (idxs1, idxs2)
using StereoMatchingOutput = std::pair<std::vector<int>, std::vector<int>>;

// (img, img_right, des, des_r, kps, kps_r, ratio_test, row_matching, max_disparity)
// -> (idxs1, idxs2)
using StereoMatchingCallback = std::function<StereoMatchingOutput(
    const cv::Mat &, const cv::Mat &, const cv::Mat &, const cv::Mat &, MatNx2dRef, MatNx2dRef,
    double, bool, double)>;
...
class FeatureSharedInfo {
  ...
  public:
    ...
    static StereoMatchingCallback stereo_matching_callback;
  ...
  public:
    ...
    static void set_stereo_matching_callback(const StereoMatchingCallback &stereo_matching_cb) {
