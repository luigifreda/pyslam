...
class FrameBase {
  ...
  public:
    ...
    std::vector<Eigen::Vector3d> transform_points(const std::vector<Eigen::Vector3d> &points) const;

    std::pair<std::vector<cv::Point2f>, std::vector<double>>
    project_points(const std::vector<Eigen::Vector3d> &points,
                   bool do_stereo_project = false) const;
    std::pair<std::vector<cv::Point2f>, std::vector<double>>
    project_map_points(const std::vector<MapPoint *> &map_points,
                       bool do_stereo_project = false) const;
    std::pair<cv::Point2f, double> project_point(const Eigen::Vector3d &pw) const;
    std::pair<cv::Point2f, double> project_map_point(const MapPoint *map_point) const;

    bool is_in_image(const cv::Point2f &uv, float z) const;
    std::vector<bool> are_in_image(const std::vector<cv::Point2f> &uvs,
                                   const std::vector<double> &zs) const;
    std::tuple<bool, cv::Point2f, float> is_visible(const MapPoint *map_point) const;
...
class Frame : public FrameBase {
  ...
  public:
    ...
    // cv::KeyPoint())
    std::vector<cv::Point2f> kps;   // left keypoint coordinates [Nx2]
    std::vector<cv::Point2f> kps_r; // right keypoint coordinates (extracted on right image) [Nx2]
    std::vector<cv::Point2f> kpsu;  // [u]ndistorted keypoint coordinates [Nx2]
    std::vector<cv::Point2f> kpsn;  // [n]ormalized keypoint coordinates [Nx2] (Kinv * [kp,1])
    ...
    void compute_stereo_from_rgbd(const std::vector<cv::Point2f> &kps_data, const cv::Mat &depth);
    ...
    float compute_points_median_depth(const std::vector<Eigen::Vector3d> *points3d = nullptr,
