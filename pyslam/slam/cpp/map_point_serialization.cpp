...
std::string MapPoint::to_json() const {
    ...
    // Observations (convert to ID references)
    {
        ...
        for (const auto &obs : _observations) {
            ...
            int idx = obs.second;
            if (kf != nullptr) {
                observations_array.push_back({kf->kid, idx});
            }
        }
    ...
    // Descriptor
    if (!des.empty()) {
        json_obj["des"] = cv_mat_to_json(des);
    } else {
    ...
    json_obj["kf_ref"] = (kf_ref != nullptr) ? kf_ref->kid : -1;

    // Semantic descriptor
    json_obj["semantic_des"] =
        serialize_semantic_des(semantic_des, FeatureSharedInfo::semantic_feature_type);

    return json_obj.dump();
}

std::unique_ptr<MapPoint> MapPoint::from_json(const std::string &json_str) {
    nlohmann::json json_obj = nlohmann::json::parse(json_str);

    // Parse basic fields
    int id = safe_json_get(json_obj, "id", -1);
    Eigen::Vector3d pt = safe_parse_vector3d(json_obj, "pt");
    ...
    auto p = std::make_unique<MapPoint>(pt, color, nullptr, -1, id);
    ...
    if (json_obj.contains("des") && !json_obj["des"].is_null()) {
        p->des = json_to_cv_mat(json_obj["des"]);
    }
    ...
    // Parse normal
    p->normal = safe_parse_vector3d(json_obj, "normal");
...
void MapPoint::replace_ids_with_objects(const std::vector<MapPoint *> &points,
                                        const std::vector<Frame *> &frames,
                                        const std::vector<KeyFrame *> &keyframes) {
    // Pre-build dictionaries for efficient lookups
    std::unordered_map<int, KeyFrame *> keyframes_dict;
    std::unordered_map<int, Frame *> frames_dict;
    ...
    auto get_keyframe_with_id = [&keyframes_dict](int id) -> KeyFrame * {
        auto it = keyframes_dict.find(id);
        return (it != keyframes_dict.end()) ? it->second : nullptr;
    };

    auto get_frame_with_id = [&frames_dict](int id) -> Frame * {
    ...
    // Replace _observations from ID-based data
    {
        ...
        for (const auto &[kf_id, idx] : _observations_id_data) {
            KeyFrame *kf = get_keyframe_with_id(kf_id);
            if (kf != nullptr) {
    ...
    // Replace _frame_views from ID-based data
    {
        ...
        for (const auto &[frame_id, idx] : _frame_views_id_data) {
            Frame *frame = get_frame_with_id(frame_id);
            if (frame != nullptr) {
...
py::tuple MapPoint::state_tuple() const {
    int version = 1;

    // Observations: KeyFrame objects + idx
    std::vector<std::pair<std::shared_ptr<KeyFrame>, int>> obs;
    obs.reserve(_observations.size());
    for (auto &kv : _observations) {
        if (kv.first) {
            obs.emplace_back(
                std::static_pointer_cast<KeyFrame>(kv.first->Frame::shared_from_this()), kv.second);
        }
    }

    // Frame views: Frame objects + idx
    std::vector<std::pair<std::shared_ptr<Frame>, int>> fviews;
    fviews.reserve(_frame_views.size());
    for (auto &kv : _frame_views) {
        if (kv.first) {
            fviews.emplace_back(std::static_pointer_cast<Frame>(kv.first->shared_from_this()),
                                kv.second);
        }
    }
    ...
    return py::make_tuple(
        ...
        _min_distance, _max_distance, py::array(3, color.data()), cvmat_to_numpy(des),
        cvmat_to_numpy(semantic_des), first_kid, obs, fviews,
        kf_ref ? std::static_pointer_cast<KeyFrame>(kf_ref->Frame::shared_from_this())
               : std::shared_ptr<KeyFrame>(),
        py::make_tuple(num_observations_on_last_update_des, num_observations_on_last_update_normals,
                       num_observations_on_last_update_semantics),
        pt_GBA, GBA_kf_id);
}

void MapPoint::restore_from_state(const py::tuple &t) {
    int idx = 0;
    ...
    using KFObs = std::vector<std::pair<std::shared_ptr<KeyFrame>, int>>;
    using FObs = std::vector<std::pair<std::shared_ptr<Frame>, int>>;
    KFObs obs = t[idx++].cast<KFObs>();
    FObs fviews = t[idx++].cast<FObs>();
    std::shared_ptr<KeyFrame> kfref = t[idx++].cast<std::shared_ptr<KeyFrame>>();

    py::tuple counters = t[idx++].cast<py::tuple>();
    num_observations_on_last_update_des = counters[0].cast<int>();
    num_observations_on_last_update_normals = counters[1].cast<int>();
    num_observations_on_last_update_semantics = counters[2].cast<int>();
    ...
    // Reattach observations and frame views
    _observations.clear();
    for (auto &kv : obs) {
        if (kv.first)
            _observations[kv.first.get()] = kv.second;
    }

    _frame_views.clear();
    for (auto &kv : fviews) {
        if (kv.first)
            _frame_views[kv.first.get()] = kv.second;
    }

    kf_ref = kfref ? kfref.get() : nullptr;
}
