...
std::string MapPoint::to_json() const {
    ...
    json_obj["kf_ref"] = kf_ref ? kf_ref->kid : -1;
...
py::tuple MapPoint::state_tuple() const {
    ...
    std::vector<std::pair<KeyFramePtr, int>> obs;
    obs.reserve(_observations.size());
    for (auto &kv : _observations) {
        if (kv.first) {
            obs.emplace_back(kv.first, kv.second);
        }
    }

    // Frame views: Frame objects + idx
    std::vector<std::pair<FramePtr, int>> fviews;
    fviews.reserve(_frame_views.size());
    for (auto &kv : _frame_views) {
        if (kv.first) {
            fviews.emplace_back(kv.first, kv.second);
        }
    }
    ...
    return py::make_tuple(
        ...
        cvmat_to_numpy(semantic_des), first_kid, obs, fviews, kf_ref ? kf_ref : nullptr,
...
void MapPoint::restore_from_state(const py::tuple &t) {
    ...
    for (auto &kv : obs) {
        if (kv.first)
            _observations[kv.first] = kv.second;
    }

    _frame_views.clear();
    for (auto &kv : fviews) {
        if (kv.first)
            _frame_views[kv.first] = kv.second;
    }

    kf_ref = kfref ? kfref : nullptr;
}
