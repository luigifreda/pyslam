...
std::vector<std::pair<KeyFrame *, int>> MapPointBase::observations() const {
    ...
    return std::vector<std::pair<KeyFrame *, int>>(_observations.begin(), _observations.end());
}

std::vector<std::pair<KeyFrame *, int>> MapPointBase::observations_iter() const {
    return std::vector<std::pair<KeyFrame *, int>>(_observations.begin(), _observations.end());
}
...
std::pair<bool, bool> MapPointBase::add_observation_if_not_bad(KeyFrame *keyframe, int idx) {
    std::lock_guard<std::mutex> lock(_lock_features);
    if (_is_bad) {
...
std::vector<std::pair<Frame *, int>> MapPointBase::frame_views() const {
    std::lock_guard<std::mutex> lock(_lock_features);
    return std::vector<std::pair<Frame *, int>>(_frame_views.begin(), _frame_views.end());
}
...
void MapPoint::delete_point() {
...

    {
        ...
        _num_observations = 0;
        observations =
            std::vector<std::pair<KeyFrame *, int>>(_observations.begin(), _observations.end());
        _observations.clear();
    }
...
void MapPoint::update_semantics(void *semantic_fusion_method, bool force) {
    ...
    for (const auto &obs : observations) {
        const KeyFrame *kf = obs.first;
        const int idx = obs.second;
        if (kf != nullptr && !kf->kps_sem.empty() && !kf->is_bad()) {
