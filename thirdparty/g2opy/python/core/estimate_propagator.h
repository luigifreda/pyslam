...
void delcareEstimatePropagator(py::module& m) {
    ...
    py::class_<EstimatePropagatorCost>(m, "EstimatePropagatorCost")
        .def(py::init<SparseOptimizer*>(), 
            "graph"_a,
            py::keep_alive<1, 2>())
        .def("__call__", &EstimatePropagatorCost::operator(),
            "edge"_a, "from"_a, "to"_a,
            py::keep_alive<1, 2>(),
            py::keep_alive<1, 3>(),
            py::keep_alive<1, 4>())   // (OptimizableGraph::Edge* edge, const OptimizableGraph::VertexSet& from, OptimizableGraph::Vertex* to_) -> double
        .def("name", &EstimatePropagatorCost::name)
    ;


    py::class_<EstimatePropagatorCostOdometry, EstimatePropagatorCost>(m, "EstimatePropagatorCostOdometry")
        .def(py::init<SparseOptimizer*>(), 
            "graph"_a,
            py::keep_alive<1, 2>())
        .def("__call__", &EstimatePropagatorCost::operator(),
    ...
    py::class_<EstimatePropagator> cls(m, "EstimatePropagator");
        ...
        py::class_<EstimatePropagator::VertexIDHashFunction>(cls, "EstimatePropagatorVertexIDHashFunction")
            .def("__call__", &EstimatePropagator::VertexIDHashFunction::operator(),
            "v"_a,
            py::keep_alive<1, 2>())   // (const OptimizableGraph::Vertex* v) -> size_t
        ;

        cls.def(py::init<OptimizableGraph*>(),
            "g"_a,
            py::keep_alive<1, 2>());
        ...
        cls.def("propagate", (void (EstimatePropagator::*) (
