...
template <typename Scalar> class SO3 {
  public:
    ...
    EIGEN_DEVICE_FUNC Eigen::Matrix<Scalar, 4, 4> orthogonal_projector() const {
        ...
        J.template block<3, 3>(0, 0) = 0.5 * (unit_quaternion.w() * Matrix3::Identity() +
    ...
    EIGEN_DEVICE_FUNC static SO3<Scalar> Exp(Tangent const &phi) {
        ...
        Quaternion q(real_factor, imag_factor * phi.x(), imag_factor * phi.y(),
                     imag_factor * phi.z());
        return SO3<Scalar>(q);
    }
    ...
    EIGEN_DEVICE_FUNC static Adjoint left_jacobian(Tangent const &phi) {
        ...
        Scalar coef1 = (theta < EPS) ? Scalar(1.0 / 2.0) - Scalar(1.0 / 24.0) * theta2
                                     : (1.0 - cos(theta)) / theta2;

        Scalar coef2 = (theta < EPS) ? Scalar(1.0 / 6.0) - Scalar(1.0 / 120.0) * theta2
