...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        default=config.system_state_folder_path + "_dense_reconstruction",
        help="Path to save the system state with the dense reconstruction",
    )
    ...
    if groundtruth is not None:
        ...
        viewer3D.set_gt_trajectory(
            gt_traj3d,
            gt_timestamps,
            align_with_scale=slam.sensor_type == SensorType.MONOCULAR,
        )
    ...
    if num_map_keyframes > 0:
        ...
        for kf in keyframes:
            ...
            print("-----------------------------------")
            print(
                f"inserting keyframe {i}/{num_map_keyframes}: {kf.id}, img_id: {kf.img_id}. img shape: {kf.img.shape}, depth shape: {kf.depth_img.shape if kf.depth_img is not None else None} type: {kf.depth_img.dtype if kf.depth_img is not None else None}, lba_count: {kf.lba_count}"
            )
